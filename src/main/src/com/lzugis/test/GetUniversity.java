...
public class GetUniversity extends CommonDao {
    ...
    public String getUniversityInfo(int startNo){
        StringBuffer sql = new StringBuffer();
        sql.append("insert into gx_university(name, city_id, record, features, institute, lon, lat)");
        ...
        try{
            ...
            for(int i=1;i<universities.size();i++){
                ...
                String city = (String) cityMap.get(elements.get(1).html());
                String record = elements.get(4).html();

                String features = "";
                Elements eleFeats = elements.get(5).children();
                if(eleFeats.size()>0) {
                    String feat985 = eleFeats.get(0).html(),
                            feat211 = eleFeats.get(1).html();
                    if (!StringUtil.isEmpty(feat985)) features += feat985 + ",";
                    if (!StringUtil.isEmpty(feat211)) features += feat211;
                }

                Elements eleInst = elements.get(6).children();
                int institute = eleInst.size()>0?1:0;

                String[] lonLat = getLonLat(name);

                sql.append("\r\nselect '"+name+"', "+city+", '"+record+"', '"+
                        features+"', "+institute+", "+lonLat[0]+", "+ lonLat[1]);
    ...
    public String[] getLonLat(String name){
        ...
        String geoUrl = "http://api.tianditu.gov.cn/apiserver/ajaxproxy?proxyReqUrl=http://api.tianditu.gov.cn/search?postStr={'keyWord':'"+name+"','level':'5','mapBound':'70.1553666510582,19.864046606779098,137.65536665105822,53.2185387942791','queryType':'1','start':'0','count':'1'}&type=query";
...
//            System.out.println(universitySql);
